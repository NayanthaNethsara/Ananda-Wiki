...
if (isset($name)) {
    if (!empty($name)) {
        ...
        if ($extension=="jpg" or $extension=="png" or $extension=="jpge") {
            ...
            $location ='../../img/';
            $picname = $sname."gallery5.".$extension;

            if(move_uploaded_file($tmp_name,$location.$picname)){
                global $extension;
                        $picture =  "img/".$sname."gallery5.".$extension;  

                        $query = "UPDATE images SET gallery5 = '{$picture}' WHERE sname ='{$sname}'";
                        $result = mysqli_query($db, $query);
                        if ($result == 1) {
                            # code...
                        
                        header('location:admin20.php');
                        }   
                    }else{
                $error = 1;
            }
