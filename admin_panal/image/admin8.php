...
if (isset($name)) {
    if (!empty($name)) {
        ...
        if ($extension=="jpg" or $extension=="png" or $extension=="jpge") {
            ...
            $location ='../../img/';
            $picname = $sname."history2.".$extension;

            if(move_uploaded_file($tmp_name,$location.$picname)){
                global $extension;
                        $picture =  "img/".$sname."history2.".$extension;  

                        $query = "UPDATE images SET his_img2 = '{$picture}' WHERE sname ='{$sname}'";
                        $result = mysqli_query($db, $query);
                        if ($result == 1) {
                            # code...
                        
                        header('location:admin9.php');
                        }   
                    }else{
                $error = 1;
            }
